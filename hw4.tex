...
\part Consider the base case of an array of length $0$. $T(1)=O(1)$ since it takes constant time to kind the $1^{st}$ smallest element in the array. Since the recurrence is bounded as 
\[ T(n)  \le \max\{ T(\text{length}(B)), T(\text{length}(C)) \} + O(n). \]
...
Consider any vector $\in \{0,1\}^n$. Following is an inductive proof for the statement that the number of vectors with odd $1$'s is the same as the number of vectors with even $1$'s. Consider a $1$-digit vector. It is clear that the statement is true with the only two possible cases being $0$ and $1$. Assume that the statement is true for a vector of length $k$. If we take the set of all $k$-dimensional vectors with odd number of $1$'s and create two sets - one by adding $1$ to the beginning of each element in the set and another by adding $0$ to the beginning of each element in the set to create two sets - the first with even number of $1$'s and the second with odd number of $1$'s. These two sets will be the same size. We can repeat the exercise by taking the set of all $k$-dimensional vectors with even number of $1$'s. So we will have two sets of the same size of $k$ - one with odd number of $1$'s and the other with even number of $1$'s. So the statement is true for $k+1$ length vectors whenever it is true for $k$ length vectors. So we have inductively proved the statement.

Even though the number of inner products with an odd number of $1$'s in the terms is the same as the number of inner products with an even number of $1$'s in the terms, due to the fact that the probability of an inner product term being $1$ is less than the term being $0$, the probability of the inner product having an odd number of terms will be less than the probability of the inner product having an even number of terms. The odd probability can be found by adding the probability of each odd number between $0$ and $n$. The probability of an inner product having $k$ number of odd terms in an inner product of two n-digit vectors being ${n\choose k } \left (\frac{1}{4}\right )^k \left ( \frac{3}{4} \right ) ^{n-k}$. The even probability can be similarly found. 


Let the probability of an inner product having an odd number of $1$'s be $\frac{a}{b}$, where $\frac{a}{b}<\frac{1}{2}$. The the probability of the inner product having an even number of $1$'s is $1-\frac{a}{b}$. For two inner product mod $2$ to be equal, they both need to have odd number of $1$'s or both need to have even number of $1$'s.

\begin{equation*}
\begin{aligned}
\Pr \left [ \text{both have odd number of  } 1\text{'s or both have even number of } 1\text{'s} \right ]  &= \frac{a}{b}\frac{a}{b} + \left (1-\frac{a}{b} \right )\left (1-\frac{a}{b} \right )\\
&= \frac{2a^2-2ab +b^2}{b^2} 
\end{aligned}
\end{equation*}

If we make $\frac{2a^2-2ab +b^2}{b^2} =\frac{1}{2}$

\begin{equation*}
\begin{aligned}
\frac{2a^2-2ab +b^2}{b^2} &=\frac{1}{2}\\
4a^2-4ab +2b^2 &= b^2\\
4a^2-4ab +b^2 &= 0\\
(2a-b)^2 &=0\\
\frac{a}{b} &= \frac{1}{2}
...
Since we know that $\frac{a}{b}<\frac{1}{2}$, the above result is a contradiction and this means that $\Pr [ \langle a, x\rangle = \langle b, x\rangle~ (\text{mod } 2)] \neq 1/2$
